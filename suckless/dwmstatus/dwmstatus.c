...
int main()
{
	...
	int i = 0;
	char *essid = NULL, *ip = NULL, *bat = NULL, *load = NULL, *time = NULL,
	     *disk = NULL, *rssi = NULL;
	char *st = NULL;
	...
	for (;; sleep(1)) {
		...
		if (is_up(interface)) {
			st = smprintf(
				"%s GiB|W: (%s at %s) %s|E: down |%s|%s|%s",
				disk, rssi, essid, ip, bat, load, time);
		} else if (is_up(inter_local)) {
			st = smprintf("%s GiB|W: down|E: (%s) %s|%s|%s|%s",
				      disk, essid, ip, bat, load, time);
		} else {
			st = smprintf("%s GiB|W: down|E: down |%s|%s|%s", disk,
				      bat, load, time);
		}
